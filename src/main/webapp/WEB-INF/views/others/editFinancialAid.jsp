    ...
    /* <![CDATA[ */
              $(function() {
                	$( "#deleteButton" ).click( function(){
	 		    	var url = 'deleteFinancialAid.html?financialAidId=${financialAid.id}';
...
<c:set var="user" value="${financialAid.student}" />
...
</c:if> &gt; ${user.name}  &gt; Edit Financial Aid</h4>
</c:if>
<c:if test="${ forProfile }" >
<h3>Profile</h3>
<h4><a href="<c:url value='/' />">Home</a> &gt; ${user.name} &gt; Edit Financial Aid</h4>
</c:if>
<c:if test="${not forProfile }" >
...
<form:form modelAttribute="financialAid">
  <form:hidden path="id" />
  <form:textarea path="details" rows="5" cols="80" />
